...
################################################################################
@click.command()
@click.argument('dataset_name', type=click.Choice(['mnist', 'cifar10', 'tiny_imagenet', 'gtsrb']))
@click.argument('net_name',
                type=click.Choice(
                    ['mnist_LeNet', 'cifar10_LeNet', 'cifar10_LeNet_ELU', 'tiny_imagenet_LeNet', 'gtsrb_LeNet']))
...
def main(dataset_name, net_name, xp_path, data_path, load_config, load_model,
    ...
    # Test model
    deep_SVDD.test(xp_path, dataset, device=device, n_jobs_dataloader=n_jobs_dataloader)
    ...
    if dataset_name in ('mnist', 'cifar10', 'tiny_imagenet', 'gtsrb'):
